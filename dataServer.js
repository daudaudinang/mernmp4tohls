...
main().catch((err) => writeLog("dataServer","database","error","Connect Database",err));
...
app.post("/getListFile", authenToken, (req, res) => {
  try{
  writeLog("dataServer",req.username,"notification","REQUEST","GET LIST FILE");
  // Check trong collection User xem loại user là gì
    User.findOne({ username: req.username }, function (err, user) {
      if (err) {
        writeLog("dataServer",req.username,"error","Get ListFile",err);
        ...
        if (user.account_type === "modifier") {
          File.find().then((listFile) => {
            writeLog("dataServer",req.username,"success","Get ListFile","Lấy danh sách File thành công!");
  ...
  writeLog("dataServer","server","error","Unknown",err);
  }
});

app.post("/removeFile", authenToken, (req, res) => {
  try{
    writeLog("dataServer",req.username,"notification","REQUEST","REMOVE FILE");
      ...
      if (req.username === file.username) {
        ...
        if (fs.existsSync("./upload/" + file.username)) {
          ...
          fs.unlink(linkUpload, function (e) {
            if (err) writeLog("dataServer",req.username,"error","Remove File Input",err);
          });

          // Xoá file converted
          fs.unlink(linkConverted, function (e) {
            if (err) writeLog("dataServer",req.username,"error","Remove File Output",err);
          });

          // Xoá list segment
          let linkDirectory = "./upload/" + file.username + "/segment/";
          fs.readdir(linkDirectory, (err, files) => {
            if (err) writeLog("dataServer",req.username,"error","Remove List File Segment",err);
        ...
        file.remove(function () {
          ...
          writeLog("dataServer",req.username,"success","Remove File From Database","Xoá File thành công!");
        ...
        User.find(
          ...
          function (err, userData) {
            if (err || userData.length === 0) {
              writeLog("dataServer",req.username,"error","Remove File","Hệ thống lỗi hoặc người dùng đang cố gắng xoá tệp không phải của họ!");
              res.json({
                status: 0,
              ...
              if (fs.existsSync("./upload/" + file.username)) {
                // Xoá file upload
                fs.unlink(linkUpload, function (e) {
                  if (e) writeLog("dataServer",req.username,"error","Remove File Input",err);
                });

                // Xoá file converted
                fs.unlink(linkConverted, function (e) {
                  if (e) writeLog("dataServer",req.username,"error","Remove File Output",err);
                });
                ...
                fs.readdir(linkDirectory, (err, files) => {
                  ...
                  for (const file of files) {
                    fs.unlink(path.join(linkDirectory, file), (err) => {
                      if (err) writeLog("dataServer",req.username,"error","Remove File File Segment",err);
                    });
                  }
                });
              }
...
app.post("/uploadFile", authenToken, upload.single("video"), (req, res) => {
  try{
    writeLog("dataServer",req.username,"notification","REQUEST","UPLOAD FILE");
    ...
    mv(
      ...
      (err) => {
        if (err) {
          writeLog("dataServer",req.username,"error","Move File",err);
    ...
    ffmpeg("./upload/" + username + "/input/" + filename)
      ...
      .on("end", function (err, stdout, stderr) {
        ...
        file.save().then(() => {
          writeLog("dataServer",req.username,"success","Upload File","Upload và convert file thành công!");
...
app.listen(process.env.PORT_DATA_SERVER || 3002, () =>{
  writeLog("dataServer","server","success","Start Server","DataServer listening on: " + process.env.PORT_DATA_SERVER || 3002);
});
